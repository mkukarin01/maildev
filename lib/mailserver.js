...
mailServer.onSmtpError = function (err) {
  if (err.code === 'ECONNRESET' && err.syscall === 'read') {
    ...
    logger.warn(`Unhandled exception happened "${err.message}"`);
    logger.error(err);
 ...
 * Setup outgoing
 */
mailServer.setupOutgoing = function (host, port, user, pass, secure, ignoreTls, requireTls) {
  outgoing.setup(host, port, user, pass, secure, ignoreTls, requireTls)
}
